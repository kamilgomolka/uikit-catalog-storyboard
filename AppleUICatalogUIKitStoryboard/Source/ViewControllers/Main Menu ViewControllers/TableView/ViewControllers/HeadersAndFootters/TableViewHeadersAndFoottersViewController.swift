...
class TableViewHeadersAndFoottersViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func setupTableView() {
        ...
        tableView.register(
            UINib(nibName: sectionHeaderNibName, bundle: nil),
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        guard
            let headerView = tableView.dequeueReusableHeaderFooterView(withIdentifier: sectionHeaderNibName)
                as? TableViewSectionHeaderView
        else {
            return nil
        }

        headerView.embededContentView.backgroundColor =
            section % 2 == 0
            ? .systemGray3
            : .systemGray5
